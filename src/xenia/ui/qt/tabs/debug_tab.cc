...
DebugTab::DebugTab() : SplitTab("Debug", "DebugTab") {
  AddSidebarItem({QChar(0xE90F), "Components", CreateComponentsTab()});
  AddSidebarItem({QChar(0xE700), "Navigation", CreateNavigationTab()});
  AddSidebarItem({QChar(0xE790), "Theme", CreateThemeTab()});
  AddSidebarItem({QChar(0xE8F1), "Library", CreateLibraryTab()});
