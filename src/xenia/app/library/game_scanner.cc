...
using AsyncCallback = GameScanner::AsyncCallback;

std::vector<std::filesystem::path> GameScanner::FindGamesInPath(
...
std::vector<GameEntry> GameScanner::ScanPath(
    const std::filesystem::path& path) {
...
int GameScanner::ScanPathAsync(const std::filesystem::path& path,
...
X_STATUS GameScanner::ScanGame(const std::filesystem::path& path,
                               GameEntry* out_info) {
  if (!out_info) {
  ...
  if (nxe_entry) {
    ...
    if (XSUCCEEDED(status)) {
      ...
      if (!XSUCCEEDED(status)) {
        XELOGE("Could not parse nxeart file: {}", nxe_file->entry()->path());
        return status;
      }
    } else {
